...
class ActivitySmallestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Activity
        fields = ('id', 'slug', 'activity_title', 'destination', 'duration',
                  'price', 'priceSale', 'trip_grade', 'max_group_size', 'best_time')
...
class ActivitySmallSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Activity
        fields = ('id', 'slug', 'activity_title', 'activity_category', 'enquiries', 'location', 'duration', 'price', 'heroImg', 'coverImg', 'ratings', 'popular', 'best_selling',
                  'destination', 'activity_region', 'priceSale', 'youtube_link', 'difficulty_level', 'max_altitude', 'trip_start', 'trips_end', 'group_style', 'best_season', 'activity_type')
        depth = 1
...
class LandingActivitySmallSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Activity
        fields = ('id', 'slug', 'activity_title', 'activity_category', 'activity_region', 'location', 'duration', 'price',
                  'heroImg', 'coverImg', 'avatar_image', 'priceSale', 'ratings', 'difficulty_level', 'group_price_available', 'activity_type')

    def get_group_price_available(self, obj):
        return ActivityPricing.objects.filter(activity=obj).exists()


class ItineraryActivityJsonldSerializer(serializers.ModelSerializer):
    class Meta:
        model = ItineraryActivity
        fields = ('title',)


class ActivityJsonldSerializer(serializers.ModelSerializer):
    activity_category_name = serializers.SerializerMethodField()
    itinerary = ItineraryActivityJsonldSerializer(many=True)

    class Meta:
        model = Activity
        fields = ('activity_title', 'coverImg', 'meta_description',
                  'activity_category_name', 'itinerary', 'priceSale')

    def get_activity_category_name(self, obj):
        # Get all activity category titles as an array
        categories = obj.activity_category.all()
...
class LandingBannerActivitySmallSerializer(serializers.ModelSerializer):
    class Meta:
        model = Activity
        fields = ('id', 'slug', 'activity_title', 'location', 'duration', 'price',
                  'heroImg', 'coverImg', 'priceSale', 'ratings', 'activity_region')
        depth = 1


class LandingBannerActivitySmallSerializer2(serializers.ModelSerializer):
    activity_region = RegionActivitySerializer()

    class Meta:
        model = Activity
        fields = ('id', 'slug', 'activity_title', 'location', 'duration', 'price',
                  'heroImg', 'activity_region')
